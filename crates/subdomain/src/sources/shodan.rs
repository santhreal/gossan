...
#[async_trait]
impl SubdomainSource for Shodan {
    fn name(&self) -> &'static str { "shodan" }
    fn requires_api_key(&self) -> bool { true }
    fn api_key_name(&self) -> &'static str { "SHODAN_API_KEY" }
    fn rate_limit(&self) -> SourceRate { SourceRate::per_second(1) }
    fn discovery_source(&self) -> DiscoverySource { DiscoverySource::Shodan }
    ...
    async fn query(
        ...
        let Some(key) = crate::sources::get_api_key(config, "shodan", "SHODAN_API_KEY") else {
            return Ok(vec![]);
        };

        let url = format!("https://api.shodan.io/dns/domain/{domain}?key={key}");
