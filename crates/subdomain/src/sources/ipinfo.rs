...
#[async_trait]
impl SubdomainSource for Ipinfo {
    fn name(&self) -> &'static str { "ipinfo" }
    fn requires_api_key(&self) -> bool { true }
    fn api_key_name(&self) -> &'static str { "IPINFO_API_KEY" }
    ...
    async fn query(
        ...
        let Some(key) = crate::sources::get_api_key(config, "ipinfo", "IPINFO_API_KEY") else {
            return Ok(vec![]);
        };

        let url = format!("https://ipinfo.io/domains/{}", domain);
        ...
        if let Some(arr) = json.get("domains").and_then(|v| v.as_array()) {
            for item in arr {
