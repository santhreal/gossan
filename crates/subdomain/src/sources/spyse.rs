...
#[async_trait]
impl SubdomainSource for Spyse {
    fn name(&self) -> &'static str { "spyse" }
    fn requires_api_key(&self) -> bool { true }
    fn api_key_name(&self) -> &'static str { "SPYSE_API_KEY" }
    ...
    async fn query(
        ...
        let Some(key) = crate::sources::get_api_key(config, "spyse", "SPYSE_API_KEY") else {
            return Ok(vec![]);
        };

        let url = format!("https://spyse.com/api/data/domain/subdomain?domain={}&limit=100", domain);
        ...
        if let Some(arr) = json.get("data").and_then(|v| v.get("items")).and_then(|v| v.as_array()) {
            for item in arr {
