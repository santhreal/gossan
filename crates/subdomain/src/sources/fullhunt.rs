...
#[async_trait]
impl SubdomainSource for Fullhunt {
    fn name(&self) -> &'static str { "fullhunt" }
    fn requires_api_key(&self) -> bool { true }
    fn api_key_name(&self) -> &'static str { "FULLHUNT_API_KEY" }
    fn rate_limit(&self) -> SourceRate { SourceRate::per_second(1) }
    fn discovery_source(&self) -> DiscoverySource { DiscoverySource::FullHunt }
    ...
    async fn query(
        ...
        let Some(key) = crate::sources::get_api_key(config, "fullhunt", "FULLHUNT_API_KEY") else {
            return Ok(vec![]);
        };
        ...
        if let Some(arr) = json.get("hosts").and_then(|v| v.as_array()) {
            for item in arr {
                if let Some(v) = item.get("host").and_then(|v| v.as_str()) {
