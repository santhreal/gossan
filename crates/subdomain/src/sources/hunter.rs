...
#[async_trait]
impl SubdomainSource for Hunter {
    fn name(&self) -> &'static str { "hunter" }
    fn requires_api_key(&self) -> bool { true }
    fn api_key_name(&self) -> &'static str { "HUNTER_API_KEY" }
    fn rate_limit(&self) -> SourceRate { SourceRate::per_second(0) }
    fn discovery_source(&self) -> DiscoverySource { DiscoverySource::HunterIo }
    ...
    async fn query(
        ...
        let Some(key) = crate::sources::get_api_key(config, "hunter", "HUNTER_API_KEY") else {
            return Ok(vec![]);
        };

        let url = format!("https://api.hunter.io/v2/domain-search?domain={domain}&api_key={key}");
        ...
        if let Some(arr) = json.get("data").and_then(|v| v.get("emails")).and_then(|v| v.as_array()) {
            for item in arr {
