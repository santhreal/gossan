...
#[async_trait]
impl SubdomainSource for Securitytrails {
    fn name(&self) -> &'static str { "securitytrails" }
    fn requires_api_key(&self) -> bool { true }
    fn api_key_name(&self) -> &'static str { "SECURITYTRAILS_API_KEY" }
    fn rate_limit(&self) -> SourceRate { SourceRate::per_second(1) }
    fn discovery_source(&self) -> DiscoverySource { DiscoverySource::SecurityTrails }
    ...
    async fn query(
        ...
        let Some(key) = crate::sources::get_api_key(config, "securitytrails", "SECURITYTRAILS_API_KEY") else {
            return Ok(vec![]);
        };
...
mod tests {
    ...
    #[test]
    fn request_has_apikey_header_and_url() {
        let client = reqwest::Client::new();
        let url = format!("https://api.securitytrails.com/v1/domain/{}/subdomains", "example.com");
        let req = client.get(&url).header("APIKEY", "secret").build().unwrap();
        assert_eq!(req.url().as_str(), "https://api.securitytrails.com/v1/domain/example.com/subdomains");
