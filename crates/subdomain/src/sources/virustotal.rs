...
#[async_trait]
impl SubdomainSource for Virustotal {
    fn name(&self) -> &'static str { "virustotal" }
    fn requires_api_key(&self) -> bool { true }
    fn api_key_name(&self) -> &'static str { "VIRUSTOTAL_API_KEY" }
    fn rate_limit(&self) -> SourceRate { SourceRate::per_second(0) }
    fn discovery_source(&self) -> DiscoverySource { DiscoverySource::VirusTotal }
    ...
    async fn query(
        ...
        let Some(key) = crate::sources::get_api_key(config, "virustotal", "VIRUSTOTAL_API_KEY") else {
            return Ok(vec![]);
        };

        let url = format!("https://www.virustotal.com/vtapi/v2/domain/report?apikey={key}&domain={domain}");
