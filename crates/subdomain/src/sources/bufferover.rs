...
#[async_trait]
impl SubdomainSource for Bufferover {
    fn name(&self) -> &'static str { "bufferover" }
    fn requires_api_key(&self) -> bool { false }
    fn api_key_name(&self) -> &'static str { "" }
    fn rate_limit(&self) -> SourceRate { SourceRate::per_second(1) }
    fn discovery_source(&self) -> DiscoverySource { DiscoverySource::BufferOver }
    ...
    async fn query(
        ...
        if let Some(arr) = json.get("FDNS_A").and_then(|v| v.as_array()) {
            for item in arr {
        ...
        Ok(seen.into_iter().map(|d| Target::Domain(DomainTarget {
            domain: d,
            source: DiscoverySource::BufferOver,
        })).collect())
    }
}
