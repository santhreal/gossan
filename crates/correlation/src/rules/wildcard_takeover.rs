...
mod tests {
    ...
    fn wildcard_finding(apex: &str) -> Finding {
        Finding::builder("subdomain", apex, secfinding::Severity::Info)
    ...
    /// PRECISION: a CORS wildcard-origin finding must NOT trigger the
    /// DNS wildcard+takeover chain. Pre-fix, the `"wildcard"` substring
    /// in "CORS: wildcard origin" matched WILDCARD_SIGNALS and every
    /// CORS-misconfig target with a dangling subdomain produced a false
    /// Critical "Mass Domain Hijack".
    #[test]
    ...
    fn wildcard_takeover_fires_via_starred_target_fallback() {
        let rule = WildcardTakeoverRule;
        let findings = vec![
            Finding::builder("dns", "*.example.com", secfinding::Severity::Info)
                .title("zone observed")
                .build()
