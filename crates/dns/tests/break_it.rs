...
async fn test_max_label_length() {
    ...
    assert!(
        res1.is_ok() || res1.is_err(),
        "valid label should just return gracefully"
    );
...
async fn test_max_domain_length() {
    ...
    let invalid_domain = std::iter::repeat("a".repeat(60) + ".")
        ...
        + "com.";
    let res = resolver
        .lookup(invalid_domain.as_str(), RecordType::A)
        .await;
...
async fn test_resolver_failover() {
    ...
    assert!(
        res.is_ok(),
        "Resolver failover should work, second IP is valid"
    );
}
...
async fn test_posture_detect_google() {
    ...
    assert!(
        !findings.is_empty(),
        "google.com should generate posture findings"
    );
}
...
async fn test_axfr_module() {
    ...
    assert!(
        !findings.is_empty(),
        "AXFR should succeed on zonetransfer.me"
    );
}
...
async fn test_all_requirements() {
    ...
    for d in &domains {
        if resolver.lookup(*d, RecordType::A).await.is_ok() {
            res_count += 1;
        }
        if resolver.lookup(*d, RecordType::AAAA).await.is_ok() {
            res_count += 1;
        }
        if resolver.mx_lookup(*d).await.is_ok() {
            res_count += 1;
        }
        if resolver.txt_lookup(*d).await.is_ok() {
            res_count += 1;
        }
        if resolver.lookup(*d, RecordType::NS).await.is_ok() {
            res_count += 1;
        }
        if resolver.lookup(*d, RecordType::SOA).await.is_ok() {
            res_count += 1;
        }
    ...
    assert!(
        res_count >= 10,
        "Should have executed at least 10 basic record assertions"
    );
}
